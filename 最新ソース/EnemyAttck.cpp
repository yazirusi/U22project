...
void EnemyAttck(void) {
	...
	//DrawFormatString(50, 160, 0xffffff, "%d", EnemyAttackX);//“G‚Ì‚˜À•W
	//“G‚Ì€–S”»’è	0:€–S@1:•\¦
	if (Enemy.drawf == 0) {
		return;
	}
	...
	if (Enemy.Attck == false) {//ƒtƒ‰ƒO‚ªfalse‚ÌƒvƒŒƒCƒ„[‚ÌÀ•W‚ğ‚¢‚ê‚é
		EnemyAttackX = Enemy.x + sx;//“G‚Ì‚˜À•W‚ğ“ü‚ê‚é
		EnemyAttackY = Enemy.y;//“G‚Ì‚™À•W‚ğ“ü‚ê‚é
		Enemy.speed = 1;//“G‚ÌˆÚ“®—Ê‚ğ‰Šú‰»
	}

	if (Enemy.AttckFlg == true) {//ƒtƒ‰ƒO‚ªtrue‚È‚ç“ü‚é
	...
	if (Hitcheck(EnemyAttackX - Enemy.MoveD, EnemyAttackY, Enemy.direction,false) == 1 ||
		...
		Enemy.AttckFlg == true) {
		Enemy.AttckFlg = false;//ƒtƒ‰ƒO‚ğoff‚É‚·‚é
		EnemyAttackX = Enemy.x;//UŒ‚À•W‚ğ‰Šú‰»‚·‚é
		EnemyAttackY = Enemy.y;//UŒ‚‚ÌÀ•W‚ğ‰Šú‰»‚·‚é
